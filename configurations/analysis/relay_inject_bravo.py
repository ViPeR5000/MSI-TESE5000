#!/usr/bin/env python3
"""
relay_inject_BRAVO.py — Injecao de comando de rele PLAINTEXT (sem cifra) — fase BRAVO.
Corre NO gateway desta fase (broker EMQX em localhost:1883, rele em 192.168.20.13:80).
Publica um comando forjado em BRAVO/DCB4D90B5258/comando (MITRE ATT&CK T1565) e le o efeito
real no web UI do rele. Injeta o OPOSTO do estado atual e no fim RESTAURA.
Esperado: REJEITADA (ASCON-128, SEGURO).
"""
...
print(f"\n>>> Injecao PLAINTEXT no rele {RELAY} (fase {PREFIX}) — esperado: REJEITADA (ASCON-128, SEGURO)\n")
before = state(RELAY)
if before is None:
