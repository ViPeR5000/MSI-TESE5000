#!/usr/bin/env python3
"""
relay_inject_CHARLIE.py — Injecao de comando de rele PLAINTEXT (sem cifra) — fase CHARLIE.
Corre NO gateway desta fase (broker EMQX em localhost:1883, rele em 192.168.30.13:80).
Publica um comando forjado em CHARLIE/CC8DA20CE224/comando (MITRE ATT&CK T1565) e le o efeito
real no web UI do rele. Injeta o OPOSTO do estado atual e no fim RESTAURA.
Esperado: REJEITADA (AES-128-GCM, SEGURO).
"""
...
print(f"\n>>> Injecao PLAINTEXT no rele {RELAY} (fase {PREFIX}) — esperado: REJEITADA (AES-128-GCM, SEGURO)\n")
before = state(RELAY)
if before is None:
