...
// ==========================================
// Relé sem criptografia: recebe comandos em claro e publica o estado em claro.
// Versão plaintext do Charlie/Bravo Relay (baseline para o custo de ter cripto).
...
#define FIRMWARE_VERSION  "1.0.0-SR-PLAIN"
#define FIRMWARE_TYPE     "Alpha-Relay"
...
void set_relay(bool on, const char* origem) {
    ...
    tracked_publish(topic_status.c_str(), plain.c_str());
}
...
void mqtt_callback(char* topic, byte* payload, unsigned int length) {
    ...
    // Comando em claro: {"command":"relay_on"|"relay_off"}
    if (t == topic_command) {
        StaticJsonDocument<128> cmd;
        if (deserializeJson(cmd, blob)) { addLog("Comando: JSON inválido"); return; }
...
void handleStatus() {
    // Dashboard glassmorphic — mesmo layout dos sensores, tema azul (Alpha).
    String html = "<!DOCTYPE html><html lang='pt'><head><meta charset='UTF-8'>";
    html += "<meta name='viewport' content='width=device-width,initial-scale=1'>";
    html += "<meta http-equiv='refresh' content='5'>";
    html += "<title>ViPeR Alpha - Relay</title><style>";
    ...
    html += ".mac-badge{background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);padding:6px 14px;border-radius:20px;font-size:0.85rem;color:#38bdf8;font-family:monospace;}";
    ...
    html += "<div class='header'><h1>ViPeR Alpha • Relay</h1>";
    ...
    html += "<div class='metric-box'><div class='metric-label'>Cripto</div><div class='metric-val err' style='font-size:1rem'>Nenhuma</div></div>";
...
void handleOTAPage() {
    ...
    html += "input[type=submit]{width:100%;padding:12px;background:#38bdf8;color:#0d0e12;border:none;border-radius:6px;font-weight:bold;cursor:pointer;}</style></head><body>";
...
void setup() {
    ...
    macHex        = getMacHex();
    topic_command = "ALPHA/" + macHex + "/comando";
    topic_status  = "ALPHA/" + macHex + "/estado";
...
void loop() {
    ...
    if (now - last_telemetry > TELEMETRY_INTERVAL) {
        ...
        last_publish_micros = micros(); waiting_for_ack = true;
        tracked_publish(topic_status.c_str(), splain.c_str());
