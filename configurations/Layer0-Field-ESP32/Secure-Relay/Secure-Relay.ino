...
#define FIRMWARE_VERSION  "1.1.0-SR-ASCON128"
#define FIRMWARE_TYPE     "Secure-Relay"
#define BUILD_TIMESTAMP   __DATE__ " " __TIME__
const char* ota_user = "admin";
const char* ota_pass = "msi-tese";

// Configuração do GPIO Físico do Relé
#define RELAY_PIN 18
...
// Monitor de performance (ASCON-128) — igual aos sensores
...
void perform_pqc_key_rotation() {
    ...
    // Pedido de encapsulamento enviando o identificador do equipamento
    ...
    if (httpResponseCode == 200) {
        ...
        // Decodificar o Segredo Partilhado Base64
        size_t output_len = 0;
        ...
        Serial.printf("[PQC] Sucesso! Segredo Quântico de %d bytes estabelecido.\n", output_len);
        
        // Atualizar chaves na cifra leve ASCON-128
        asconLwc.setKey(shared_secret, output_len);
        
        pqc_handshake_completed = true;
        last_key_rotation = millis();
        hs_pending = true;
        Serial.println("[PQC] Chave LWC ASCON-128 atualizada de forma segura.");
    } else {
...
// Envia o estado atual do relé encriptado via ASCON para o Broker MQTT
// Aplica um estado ao relé físico e publica o estado cifrado (com loopback p/ latência)
void set_relay(bool on, const char* origem) {
    current_relay_state = on;
    digitalWrite(RELAY_PIN, on ? HIGH : LOW);
    addLog(String("Relé ") + (on ? "LIGADO" : "DESLIGADO") + " (" + origem + ")");
    if (!pqc_handshake_completed) return;
    ...
    String enc = asconLwc.encrypt(plain);
...
// Callback: loopback do próprio estado (latência) + comando cifrado ASCON
void mqtt_callback(char* topic, byte* payload, unsigned int length) {
    ...
    if (t == topic_command && pqc_handshake_completed) {
        String pt = asconLwc.decrypt(blob);
        if (pt.length() == 0) { addLog("Comando: decrypt ASCON falhou"); return; }
        StaticJsonDocument<256> doc;
        if (deserializeJson(doc, pt)) { addLog("Comando: JSON inválido"); return; }
        // Aceita {"relay":1/0} (Node-RED Bravo) ou {"command":"relay_on/off"} (e-paper)
        if (doc.containsKey("relay")) {
            set_relay(doc["relay"].as<int>() == 1, "MQTT");
        } else if (doc.containsKey("command")) {
            String c = String((const char*)(doc["command"] | ""));
...
void connect_mqtt() {
    while (!mqttClient.connected()) {
        Serial.printf("[EMQX] Tentando conectar como '%s'...", macHex.c_str());
        if (mqttClient.connect(macHex.c_str())) {
            Serial.println(" Sucesso na ligação!");
...
void handleStatus() {
    // Dashboard glassmorphic — mesmo layout dos sensores, tema branco (Bravo).
    String html = "<!DOCTYPE html><html lang='pt'><head><meta charset='UTF-8'>";
    html += "<meta name='viewport' content='width=device-width,initial-scale=1'>";
    html += "<meta http-equiv='refresh' content='5'>";
    html += "<title>ViPeR Bravo - Relay</title><style>";
    ...
    html += ".mac-badge{background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);padding:6px 14px;border-radius:20px;font-size:0.85rem;color:#e2e8f0;font-family:monospace;}";
    ...
    html += "<div class='header'><h1>ViPeR Bravo • Relay</h1>";
...
void handleRelay() {
    if (statusServer.hasArg("state")) set_relay(statusServer.arg("state") == "on", "web");
...
void setup() {
    Serial.begin(115200);
    delay(1000);
    Serial.println("\n\n=== INICIANDO ACTUADOR DE RELÉ SEGURO (SECURE-RELAY) ===");
    Serial.println("[FW] Versão  : " FIRMWARE_VERSION);
    Serial.println("[FW] Build   : " BUILD_TIMESTAMP);

    // Configurar pino físico do Relé como saída
    pinMode(RELAY_PIN, OUTPUT);
    digitalWrite(RELAY_PIN, LOW); // Inicia desligado por segurança

    // Obter MAC Address Único e inicializar tópicos dinâmicos
    macHex = getMacHex();
    topic_command = "BRAVO/" + macHex + "/comando";
    topic_status = "BRAVO/" + macHex + "/estado";
...
void loop() {
    ...
    if (pqc_handshake_completed && (now - last_telemetry > TELEMETRY_INTERVAL)) {
        last_telemetry = now;
        ...
        uint32_t h0 = ESP.getFreeHeap();
        unsigned long e0 = micros();
        String enc = asconLwc.encrypt(splain);
        crypto_cpu_overhead_us = (float)(micros() - e0);
        crypto_ram_overhead_kb = max(0.0f, (float)(h0 - ESP.getFreeHeap()) / 1024.0f);
        last_publish_micros = micros(); waiting_for_ack = true;
        tracked_publish(topic_status.c_str(), enc.c_str());
        ...
        String enc_m = asconLwc.encrypt(mplain);
